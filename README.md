...
The robot’s environment will include one lamp at a height of 2 m (this acts as the target). The robot stops moving as soon as it approaches the projection of the lamp on the ground at a distance equal to or less than 0.6 m. (Obviously, it was needed to conduct experiments to determine the brightness values collected by the sensors when they are this close to the lamp, in order to use them as a stopping condition.)
